...
> **📡 网段与区域（拓扑无关）**：本平台对网络拓扑**没有任何要求**——打印机可以全在**同一个网段**，也可以分散在**多个网段 / 多个楼层 / 多个办公室**。每台打印机只需要一个「运行监控的机器能 ping 通」且已开启 SNMP 的 IP；`location` 是**自由文本**（如 `财务室` / `仓库` / `一楼前台`），只用于面板分区与告警定位。
> **只有一台打印机、或只有一个网段，照常使用，无需任何额外配置**——把真实 IP 填进去即可，例子里不同网段的写法只是为了演示「跨网段也支持」。
